...
# Load the GT file (update path if needed)
...
for part in parts:
    for i in range(0, 133):  # Up to 133 keypoints
        header.extend([f'{part}_x_{i}', f'{part}_y_{i}', f'{part}_v_{i}'])

with open('example_gt.csv', 'w', newline='') as f:
    writer = csv.writer(f)
...
print("✅ CSV file created: example_gt.csv")
